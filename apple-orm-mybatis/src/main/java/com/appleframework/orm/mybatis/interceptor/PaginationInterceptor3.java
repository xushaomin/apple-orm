...
public class PaginationInterceptor3 extends PaginationHelper implements Interceptor {

	private static Logger logger = LoggerFactory.getLogger(PaginationInterceptor3.class);
	...
	public Object intercept(Invocation ivk) throws Throwable {
		...
		if ("sqlserver".equals(dialect)) {
			...
			countSql = getSimpleCountSql(sql);
		}
	...
	private String generatePageSql(String sql, SimplePage page) {
		if (page != null && (dialect != null || !dialect.equals(""))) {
			...
			if ("mysql".equals(dialect)) {
				...
				if(index > 0) {
					pageSql.append(sql.substring(0,sql.indexOf(";")));
				}else {
					pageSql.append(sql);
				}
				pageSql.append(" LIMIT " + page.getFirstResult() + "," + page.getPageSize());
				if(index > 0) {
					pageSql.append(sql.substring(sql.indexOf(";"),sql.length()));
				}
	...
	public static String getSimpleCountSql(final String sql) {
        ...
        if(index > 0) {
        	stringBuilder.append(sql.substring(0,sql.indexOf(";")));
        }else {
        ...
        if(index > 0) {
        	stringBuilder.append(sql.substring(sql.indexOf(";"),sql.length()));
        }
